...
function bluetooth_connected_devices {

  local text=$(bluetoothctl devices Connected | cut -f2 -d' ' | while read uuid; do bluetoothctl info $uuid; done|grep -e "Name" | cut -f2 -d':')
  if [[ -z "$text" ]] ; then
    json_array=$(remove_holder holder__bluetooth_connected_devices)
  else
    local json='{ "full_text": "'󰂰"$text"'", "color": "#0082FC" }'
    json_array=$(update_holder holder__bluetooth_connected_devices "$json")
  fi
